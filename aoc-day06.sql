create table aoc_day06 (response varchar(26));

insert into aoc_day06 (response) values
('psyjxulrdtfe')
,('jeusdrlxyftp')
,('ufdpjsxrlztye')
,('yeorabxsdnhftujlp')
...
create table if not exists response_combined (responses varchar(255));
truncate table response_combined;
...
begin
    for rec in 
        select response from aoc_day06
    loop
        if rec.response = '' then
            insert into response_combined (responses) values (r_text);
            r_text := '';
            continue;
        end if;

        r_text := r_text || rec.response;
    end loop;
end;
$$

create or replace function distinct_char_count(str varchar(255))
    returns bigint as
$$
declare
    rv int;
begin
    rv := (select count(*) from (select distinct unnest(arr_char) from (select string_to_array(str, null) as arr_char) a) b); 
    return rv;
end;
$$
language plpgsql;

select sum(distinct_char_count(responses)) from response_combined;
/*
 sum  
------
 6686
...
/* PART 2 */
create table if not exists response_combined_groups (num_in_group int, responses varchar(255));
truncate table response_combined_groups;
...
begin
    ...
    loop
        if rec.response = '' then
            insert into response_combined_groups (num_in_group, responses) values (num, r_text);
            r_text := '';
            num = 0;
            continue;
        end if;
...
from (select unnest(arr_char) as char
        from (select string_to_array(str, null) as arr_char) a
        ) b
group by 1
having count(*) = group_num
) c); 
    return rv;
end;
$$
