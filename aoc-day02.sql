create table aoc_day02 (password varchar(255));

insert into aoc_day02 values 
...
/* part 1 */
with password_data as (
    select 
    cast(split_part(split_part(password, ' ', 1), '-', 1) as int) as min_v
    , cast(split_part(split_part(password, ' ', 1), '-', 2) as int) as max_v
    , trim(trailing ':' from split_part(password, ' ', 2)) as char_v
    , split_part(password, ' ', 3) as password 
    from aoc_day02)
select count(*)
from password_data
where char_length(password) - char_length(replace(password, char_v, '')) between min_v and max_v;
/* 
 count 
-------
   546
...
select count(*)
from password_data
where (substring(password from min_v for 1) = char_v or substring(password from max_v for 1) = char_v)
and not (substring(password from min_v for 1) = char_v and substring(password from max_v for 1) = char_v);
/*
 count 
-------
   275
