create table aoc_day04 (passport varchar(255));

insert into aoc_day04(passport) values
('iyr:2016 cid:189 ecl:oth byr:1947 hcl:#6c4ab1 eyr:2026')
,('hgt:174cm')
,('pid:526744288')
,('')
...
create table if not exists passport_combined (line varchar(255));
truncate table passport_combined;
...
begin
    for rec in 
        select passport from aoc_day04
    loop
        if rec.passport = '' then
            insert into passport_combined (line) values (pp_text);
            pp_text := '';
            continue;
        end if;

        pp_text := pp_text || ' ' || rec.passport;
    end loop;
end;
$$

/* clean up */
update passport_combined set line = trim(line);
